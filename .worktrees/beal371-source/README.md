...
> **v7.0.0-genuine-provider — 0 `sorry`, two named domain axioms plus explicit provider data**
>
...
> important v7 change is the shape of the Ribet boundary: the former opaque
> B15 single-step axiom has been deleted from the active source path and
> replaced by explicit data-valued provider functions.
>
...
> → **an explicit `RibetSingleStepProviders` family**
> → **level 2** → **$S_2(Γ_0(2)) = 0$**
> → **contradiction**.
>
...
> `Beal.RibetIterate.RibetSingleStepProviders`.
...
| `RibetSingleStepProviders` | Explicit family of complete data-valued B15 step providers, one for each residual prime. |
...
the provider family remains an explicit missing data boundary.
---
...
explicit data-valued provider family:
...
3. **Genuine provider:** `RibetSingleStepProviders` supplies each complete
   data-valued B15 step. Its one-edge Galois-facing constructor is
   `ribet_single_step_from_genuine`, using
   `SupportedNewformToTokenProvider`.
...
interfaces and explicit provider family mark the remaining classical inputs;
...
`RibetSingleStepProviders` family. The 07k one-edge constructor shows how a
`SupportedNewformToTokenProvider` supplies the target token from genuine
support. `B16_BealFinal.lean` then eliminates the terminal token using the
verified equality $S_2(\Gamma_0(2)) = 0$.

The current B15 plan still carries arithmetic edge data only. It cannot
instantiate `hIhara`, `hOldNew`, or `hRank` per edge until the plan is extended
with the corresponding Galois witnesses. The explicit provider family keeps
that missing construction visible rather than hiding it in an axiom.
...
interfaces and the explicit provider family. Replacing those boundaries with
...
- **Audit final B20 declarations** — `#print axioms` must expose
  ...
  explicit `RibetSingleStepProviders` parameter rather than a named axiom
