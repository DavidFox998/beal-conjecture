...
/-- The external soundness boundary for the archived v9.2.0 mwrank transcript.

Lean checks the exact model, quartic, rank, and Selmer-rank fields of the
transcript in `SecondDescent_Real_26.lean`.  This proposition is the remaining
mathematical assertion that mwrank's reported zero Selmer ranks produce the
proof-relevant complete descents and soundness maps required by
`SecondDescentHypothesis_26`.  It is intentionally a premise, not an axiom or a
definition reducible by `decide`. -/
...
theorem SecondDescentHypothesis_26_real
    (hMwrank : MwrankCertificateSoundness_26) :
