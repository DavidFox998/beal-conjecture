...
computation.  This module provides two separate routes to cardinality one:
one from the explicit full-Selmer triviality fields in
`SecondDescentCertificate_26`, and one from a genuine equivalence between the
cohomological Selmer carrier and the surviving locally soluble ledger classes.
No Mordell--Weil rank statement is used in either route.
-/
...
/-- The numerical Selmer endpoint follows from a genuine equivalence with the
surviving locally soluble ledger classes.  The equivalence is deliberately an
explicit input: this theorem does not identify a coefficient ledger with a
cohomological Selmer group by fiat. -/
